<script setup lang="ts">
import SubTitle from '@/components/ui/SubTitle.vue';
</script>

<template>
    <div
        class=" bg-gradient-to-r from-[#EFA3A7] via-[#f8c7cb] to-[#EFA3A7] rounded-[10px] md:mt-[130px] md:p-20 p-10  relative">
        <h1 class="lg:text-[80px] md:text-[60px] ibm-plex-sans-semibold tracking-[1px] sm:text-[40px] text-[30px] text-center font-bold text-white md:tracking-[5px] pb-10">
            MA'LUMOTLAR</h1>
            <div class=" h-full relative ">
                <img class="absolute lg:bl md:block hidden bottom-[-200px] md:bottom-[-200px] left-1/2 transform -translate-x-1/2 md:h-[750px] h-[980px] object-cover z-[10]"
                    src="https://ik.imagekit.io/vtroph5l9/Product/121.png?updatedAt=1735399271048" alt="" />
                <div class="relative  grid md:grid-cols-3 grid-cols-1 h-full gap-5">
                    <!-- Left side -->
                    <ul class="flex flex-col items-center space-y-5 max-w-full">
                        <div class="flex items-center space-x-5">
                            <li class="p-5 rounded-[50%] bg-white">
                                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                                    xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                                    viewBox="0 0 24 24">
                                    <path
                                        d="M13.849 4.22c-.684-1.626-3.014-1.626-3.698 0L8.397 8.387l-4.552.361c-1.775.14-2.495 2.331-1.142 3.477l3.468 2.937-1.06 4.392c-.413 1.713 1.472 3.067 2.992 2.149L12 19.35l3.897 2.354c1.52.918 3.405-.436 2.992-2.15l-1.06-4.39 3.468-2.938c1.353-1.146.633-3.336-1.142-3.477l-4.552-.36-1.754-4.17Z" />
                                </svg>
                            </li>
                            <p class="text-[15px] text-black">took a galley of type and scrambled it to make a type
                                specime
                            </p>
                        </div>
                        <div class="flex items-center space-x-5">
                            <li class="p-5 rounded-[50%] bg-white">
                                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                                    xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                                    viewBox="0 0 24 24">
                                    <path
                                        d="M13.849 4.22c-.684-1.626-3.014-1.626-3.698 0L8.397 8.387l-4.552.361c-1.775.14-2.495 2.331-1.142 3.477l3.468 2.937-1.06 4.392c-.413 1.713 1.472 3.067 2.992 2.149L12 19.35l3.897 2.354c1.52.918 3.405-.436 2.992-2.15l-1.06-4.39 3.468-2.938c1.353-1.146.633-3.336-1.142-3.477l-4.552-.36-1.754-4.17Z" />
                                </svg>

                            </li>
                            <p class="text-[15px] text-black">took a galley of type and scrambled it to make a type
                                specime
                            </p>
                        </div>
                        <div class="flex items-center space-x-5">
                            <li class="p-5 rounded-[50%] bg-white">
                                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                                    xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                                    viewBox="0 0 24 24">
                                    <path
                                        d="M13.849 4.22c-.684-1.626-3.014-1.626-3.698 0L8.397 8.387l-4.552.361c-1.775.14-2.495 2.331-1.142 3.477l3.468 2.937-1.06 4.392c-.413 1.713 1.472 3.067 2.992 2.149L12 19.35l3.897 2.354c1.52.918 3.405-.436 2.992-2.15l-1.06-4.39 3.468-2.938c1.353-1.146.633-3.336-1.142-3.477l-4.552-.36-1.754-4.17Z" />
                                </svg>
                            </li>
                            <p class="text-[15px] text-black">took a galley of type and scrambled it to make a type
                                specime
                            </p>
                        </div>
                    </ul>

                    <!-- Center image -->
                    <div></div>

                    <!-- Right side -->
                    <ul class="flex flex-col items-center space-y-5 max-w-full">
                        <div class="flex items-center space-x-5">
                            <p class="text-right text-black text-[15px]">took a galley of type and scrambled it to make
                                a
                                type specime
                            </p>
                            <li class="p-5 m-0 rounded-[50%] bg-white">
                                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                                    xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                                    viewBox="0 0 24 24">
                                    <path
                                        d="M13.849 4.22c-.684-1.626-3.014-1.626-3.698 0L8.397 8.387l-4.552.361c-1.775.14-2.495 2.331-1.142 3.477l3.468 2.937-1.06 4.392c-.413 1.713 1.472 3.067 2.992 2.149L12 19.35l3.897 2.354c1.52.918 3.405-.436 2.992-2.15l-1.06-4.39 3.468-2.938c1.353-1.146.633-3.336-1.142-3.477l-4.552-.36-1.754-4.17Z" />
                                </svg>
                            </li>
                        </div>
                        <div class="flex items-center space-x-5">
                            <p class="text-right text-black text-[15px]">took a galley of type and scrambled it to make
                                a
                                type specime
                            </p>
                            <li class="p-5 rounded-[50%] bg-white">
                                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                                    xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                                    viewBox="0 0 24 24">
                                    <path
                                        d="M13.849 4.22c-.684-1.626-3.014-1.626-3.698 0L8.397 8.387l-4.552.361c-1.775.14-2.495 2.331-1.142 3.477l3.468 2.937-1.06 4.392c-.413 1.713 1.472 3.067 2.992 2.149L12 19.35l3.897 2.354c1.52.918 3.405-.436 2.992-2.15l-1.06-4.39 3.468-2.938c1.353-1.146.633-3.336-1.142-3.477l-4.552-.36-1.754-4.17Z" />
                                </svg>
                            </li>
                        </div>
                        <div class="flex items-center space-x-5">
                            <p class="text-right text-black text-[15px]">took a galley of type and scrambled it to make
                                a
                                type specime
                            </p>
                            <li class="p-5 rounded-[50%] bg-white">
                                <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true"
                                    xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"
                                    viewBox="0 0 24 24">
                                    <path
                                        d="M13.849 4.22c-.684-1.626-3.014-1.626-3.698 0L8.397 8.387l-4.552.361c-1.775.14-2.495 2.331-1.142 3.477l3.468 2.937-1.06 4.392c-.413 1.713 1.472 3.067 2.992 2.149L12 19.35l3.897 2.354c1.52.918 3.405-.436 2.992-2.15l-1.06-4.39 3.468-2.938c1.353-1.146.633-3.336-1.142-3.477l-4.552-.36-1.754-4.17Z" />
                                </svg>
                            </li>
                        </div>
                    </ul>
                </div>
            </div>
    </div>
</template>


